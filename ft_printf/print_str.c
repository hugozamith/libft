...
/*   Updated: 2024/05/10 17:49:17 by hteixeir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	put_spaces(t_data *data, char *s)
{
    ...
    if (data->format.width_value > 0) {
        if (data->format.percision_value >= 0 && data->format.percision_value < len)
...
{
	if (((data->format.percision_value >= 6) ||	data->format.percision_value == -1)
		&& NULL == s)
		s = "(null)";
	else if (NULL == s)
		s = "";
    int str_len = ft_strlen(s);
    if (data->format.percision_value >= 0 && data->format.percision_value < str_len)
