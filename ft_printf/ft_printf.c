...
/*   Updated: 2024/05/10 16:01:15 by hteixeir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	write_format(t_data *data)
{
	...
	if (!ft_strchr("dipxXu", data->format.specifier))
	{
...
static int	init_data(t_data *data, const char *frmt)
{
	...
	if (data->buff == NULL)
		return (MALLOC_ERROR);
	data->buff_index = 0;
	ft_memset(data->buff, 0, BUF_SIZE * sizeof(char));
	return (OK);
}
